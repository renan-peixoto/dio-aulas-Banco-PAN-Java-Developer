...
public class Fila<T> {
  ...
  public T first() {
    if (!this.isEmpty()) { // verifica-se se a fila está vazia. Se a fila estiver vazia, o método retornará
                           // null
      No<T> primeiroNo = refNoEntradaFila; // a referência ao nó de entrada da fila é atribuída a uma variável local
      // chamada primeiroNo.
      ...
      return (T) primeiroNo.getObject(); // Retorna o valor de primeiroNo, que representa o primeiro elemento na fila.
  ...
  public T dequeue() {
    if (!this.isEmpty()) { // verificação para saber se a fila está vazia. Se a fila estiver vazia, o
                           // método retorna null
      No<T> primeiroNo = refNoEntradaFila; // Cria uma variável local primeiroNo e a inicializa com a referência ao nó
                                           // de
      ...
      return (T) primeiroNo.getObject(); // O método retorna o nó referenciado por primeiroNo, que é o primeiro nó da
      // fila.
    }
